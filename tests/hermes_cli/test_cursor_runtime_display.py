...
def test_banner_shows_cursor_pty_runtime_model_label(tmp_path):
    from hermes_cli import banner
    ...
    with (
        patch("hermes_cli.config.get_hermes_home", return_value=hh),
        patch("hermes_constants.get_hermes_home", return_value=hh),
        patch("hermes_cli.config.load_config", return_value=_cursor_pty_config()),
    ):
        buf = io.StringIO()
        console = Console(file=buf, width=400, force_terminal=False, color_system=None)
        banner.build_welcome_banner(
    ...
    out = buf.getvalue()
    assert "cursor:pty:auto" in out
    assert "Cursor PTY" in out
    assert "gpt-oss-120b" not in out
...
def test_status_bar_snapshot_shows_cursor_pty_runtime_model_label(monkeypatch):
    import cli

    monkeypatch.setattr(cli, "CLI_CONFIG", _cursor_pty_config())
    ui = object.__new__(cli.HermesCLI)
    ui.model = "openai/gpt-oss-120b"
    ui.session_start = datetime.now()
    ui._prompt_start_time = None
    ...
    assert snapshot["model_short"] == "cursor:pty:auto"
...
def test_startup_status_shows_cursor_pty_runtime_model_label(monkeypatch):
    import cli
